...
def preload_model(model):
    ...
    state_dict = torch.load('/raid/maxusen/ImageReward/train/checkpoint/blip_uni_cross_mul_bs128_fix=0.7_lr=1e-05cosine/best_lr=1e-05.pt', map_location=model.device)
    msg = model.load_state_dict(state_dict,strict=False)
    print("missing keys:", msg.missing_keys)
