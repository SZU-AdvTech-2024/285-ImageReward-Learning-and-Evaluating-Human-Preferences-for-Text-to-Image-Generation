...
class ImageReward(nn.Module):
    def __init__(self, device='cpu'):
        super().__init__()
        self.device = device
        print(config['blip_path'])
        print(config['BLIP']['vit'])
        # self.blip = blip_pretrain(pretrained=config['blip_path'], image_size=config['BLIP']['image_size'], vit=config['BLIP']['vit'])
        self.blip = blip_pretrain(pretrained='/raid/maxusen/ImageReward/train/model_large.pth', image_size=config['BLIP']['image_size'],
                                  vit=config['BLIP']['vit'])
